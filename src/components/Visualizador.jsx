...
export default function Visualizador({ chapa, pecas, largCorte, escala }) {
  ...
  listaParaPosicionar.forEach((peca, index) => {
    ...
    if (melhorEspaçoIdx !== -1) {
      ...
      // A MÁGICA: Dividir o retângulo restante de forma a manter tiras verticais longas
      const sobraDireita = {
      ...
      if (sobraDireita.w > 0 && sobraDireita.h > 0) espaçosLivres.push(sobraDireita);
      ...
      pecasPosicionadas.push({ ...peca, largura: peca.w, altura: peca.h, erro: true });
    }
  });

  const areaTotalChapa = chapa.largura * chapa.altura;
  const aproveitamento = areaTotalChapa > 0 ? ((areaUtilizada / areaTotalChapa) * 100).toFixed(1) : 0;
    ...
    <div className="container-visualizador">
      ...
      <div className="grade-estatisticas">
        ...
        <div className="card-estatistica">
          ...
          <p className="valor-destaque">{(areaTotalChapa / 1000000).toFixed(2)}m²</p>
        </div>
        <div className="card-estatistica">
          <span>Sobras</span>
          <p className="valor-destaque cor-laranja">{(100 - aproveitamento).toFixed(1)}%</p>
        </div>
      </div>
      ...
      >
        ...
        {pecasPosicionadas.map((p) => {
          ...
          return (
            ...
            >
              <span className="info-medida">{p.largura} x {p.altura}</span>
        ...
        {espaçosLivres.map((espaco, index) => (
          (espaco.w > 50 && espaco.h > 50) && (
            <div
              key={`sobra-${index}`}
            ...
            >
              <span style={{ fontSize: '8px', color: 'rgba(0,0,0,0.6)', fontWeight: 'bold' }}>
      ...
      {pecasPosicionadas.some(p => p.erro) && (
        <div style={{ marginTop: '20px', color: '#b91c1c', fontWeight: 'bold' }}>
          ⚠️ Algumas peças não couberam na chapa!
        </div>
      )}
    </div>
    <div className="secao-lista-corte">
        <h3>Lista de Corte Detalhada</h3>
        ...
        return (largura > 0 && altura > 0) && (
          <tr key={peca.id}>
            ...
            <td><strong>{largura} x {altura}</strong></td>
            <td>{qtd}</td>
            <td>{areaTotal} m²</td>
          </tr>
        );
