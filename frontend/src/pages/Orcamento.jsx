...
//     // CORREÇÃO: Garante que o contexto local seja atualizado imediatamente
//     // impedindo que o useEffect sobreescreva a tela com dados vazios.
//     atualizarContexto({
...
//       setExtras(extrasParsed.map((e, idx) => ({
//           id: Date.now() + idx,
//           descricao: e.descricao,
//           valor: Number(e.valor),
//       })));
...
//       // Filtra os dados de acordo com a situação (aberto ou gerado) vinda do botão
...
//           const render = (val) => {
//             // Filtra o que já foi separado por situação com base no que o usuário digita
...
//                 <input type="text" className="input-lupa-flex" value={nomeCliente} readOnly placeholder="Nome do cliente..."  /> {/*  disabled */}
...
//             <div className="form-group flex-1"><label className="titulo-input">Adiantamento (R$)</label><input type="text" value={formatMoney(adiantamento)} onChange={(e) => handleMoneyInput(e.target.value, setAdiantamento)} /></div>
//           </div>
//         </div>
